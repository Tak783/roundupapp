...
2. Other appoachess can work too as diffrerent projects at different stages and eng culture require different approaches.
3. A highly modular approach using Platform Agnostic components and SOLID principles was taken, focusing on simplicity and flexibily.
4. Unit and integration written using TDD, BDD and FIRST princple approcah tests were written accross the projectr
5. Tests were mostly written for Swift Packages and business logic. More would be written with more time for presentaion layer with time.
...
* There 7 layers. At the lowest level are Foundational compoents (i.e. extensions to Foundation), at the highest level is the Presentation Layer which contains the iOS app 
* Highly modular. Each the bottom 5 layers are platform agnostic, so packages can be used by either of UIKit, SwiftUI and WatchOS ect. 
* There is Vertical use of dependencies: each package can import *only* from layers below.

<img width="1280" alt="Architecture" src="https://user-images.githubusercontent.com/3129728/123437070-30de1100-d5c7-11eb-97c9-c96181880b64.png">

### Platform agnostic components
App is built with platform-agnostic components for the following reasons.
...
* Faster *build times* for tests, testing suites and project locally and on pipelines. 
...
* Each module lives in it's own independent project with as few dependencies as possible, contains its own tests, this way each feature can be
...
* CoreStarlingEngineSharedModels: Core models/interfaces to be used for Engine apps i.e. `Amountable` - used later in all of Feature Layer frameworks 

### Core Feature Layer Presentation Layer Helpers
Contains platform agnostic components and extensions used by higher layers to support their uses in the presentaion layer
...
### Core Presentaion (explained)
* CoreUIKit: contains extensions to the `UIKit` framework. Meant to only be that.
* CorePresentation: Contains Navigational components i.e. the coordinators and alerts, components core to presentaion layer. These can be used for UIKIt and SwiftUI etc.
...
* OAuth + Token manager
    ...
    * Inb the future keychain storage manager would have been used 
...
* Architecture update to use more inversion of control as part of Single Responsibility Principle (SRP)
    * Composers would be the bridges between packages that interact together instead of the View Controllers
    * The above would lead to better application of the SRP as the VC would only consume the obects leaving creation and injection to the composer
...
* Add Feature specific strings files for each feature so that each feature comes ready out of the box to have a presentation layer added
...
### Should
* Feature UI Layer
  * Move feature UI code away from the iOS app to their own independant packages so they can be more indipendent which would lead to
    * Faster test times and development times, abilty to create and use Demo Apps
    * Better application of SRP, Open Closed, and Dependency Inversion
    * More modularity as we can more easily remove them from the app or swithc the internals of the package i.e. from UIKit to SwiftUI
* Feature Packages 
...
*  Add a new `SavingsGoalFeatureTesting` Framework  move domain-specific mock data from `CoreTesting` to a new  `SavingsGoalFeatureTesting` framework i.e
